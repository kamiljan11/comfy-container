...
export const FEATURED: CaseStudy[] = [
  ...
  },
  {
    ...
    resultsPreview:
      "Scripted status calls and order SMS now run unattended inside MAS Parts' logistics flow. The two-way realtime agent works, but it's still hardening. It's safe to point at real customers because the model only phrases and routes; deterministic code performs every action it would otherwise take.",
    problem:
  ...
  },
  {
    ...
    results:
      "The morning after I finally got resume working, I came downstairs to a completed overnight queue and zero duplicates. That sounds small written down. It was the exact failure that had started the whole project, finally not happening. The job that used to die at 3am on a rate limit and leave a re-sent email behind had instead waited out the limit and picked back up on the right step.\n\nWhat I have now is a working personal agent platform that runs scheduled, memory-backed automations unattended: drafting outbound email overnight, watching repos, logging my own work sessions, and carrying context across runs instead of starting blank each time. One of the quieter residents is a weekly job-market radar: it sweeps real listings across several markets, scores them against hard gates, keeps a persistent signal ledger so expired offers still count as market data, and pushes to my phone only when something actually clears the bar.\n\nThe same guardrail philosophy extends to the one place an agent could spend real money: ad campaigns are managed as code through the Marketing API, every write is a dry-run unless explicitly confirmed, and campaigns are always created paused. An agent here can build a complete campaign: it structurally cannot start the spend. Launching is a human's click, by design.\n\nThe split between what the model phrases and what code commits has held in daily use across several of my own ventures. For one-person infrastructure, that's the only validation that counts: it keeps running without me in the loop.\n\nAnd it now improves without me driving every change. The daily self-audit turns transcripts into a handful of evidence-backed prescriptions; the ones that survive my review become standing rules; the false-positive filters those reviews produced are themselves part of the system now. One person plus a runtime that files its own maintenance tickets covers more ground than one person ever did alone.\n\nFor scale, the honest numbers as of this writing: 41 scheduled tasks defined, roughly 140 skill definitions across two agent runtimes encoding the repeatable workflows, three event hooks plus a global commit guard enforcing quality, and one dedicated reviewer subagent. Not a lab: a working operation.",
    principle:
  ...
  },
  {
    ...
    decisions: [
      ...
      },
      {
        ...
        rejected:
          "Routing everything through the model for a 'consistent experience'. Consistent, yes, but also slower and more expensive at the exact moment speed matters most.",
        tradeoff:
  ...
  },
  {
    ...
    decisions: [
      ...
      },
      {
        ...
        tradeoff:
          "Every contract stores its full text, so the same paragraphs are duplicated across rows, and fixing a typo doesn't reach old documents. When I replaced the terms I checked the documents already in the database, two signed and one sent, and confirmed they render from their stored text and were untouched.",
      },
    ],
    build:
  ...
  {
    slug: "fleet-quality-pipeline",
    title: "Quality gates for thirty repos, shipped in a day",
    resultsPreview:
      "About thirty repos across my ventures got the same floor in one rollout: a CI quality gate, AI code review, server-side branch protection, and a self-improvement loop that must delete more code than it adds. A supply-chain scan swept 1,844 files and triaged 51 hits. The best part is the failure: two weeks in, my own pinned template broke CI across the fleet, and the audit I'd scheduled caught it and said so to my face.",
    problem:
      "Every repo had its own idea of what 'done' meant. Some had tests, some had a linter, most had whatever discipline I remembered to apply the week I built them. One person maintaining that many codebases doesn't review pull requests at 2am, so quality was whatever my past self happened to enforce.\n\nAnd the risk wasn't hypothetical. These repos run real businesses: client sites, an operations platform, a workshop system. A dependency with a known CVE or a sloppy commit doesn't announce itself. It just sits there until it costs something.\n\nThe usual fix is discipline. I don't trust discipline at this scale. I trust defaults.",
    context:
      "The fleet is about thirty repositories on one GitHub account, from production platforms to small client builds. One owner, a few contractors, agents committing code daily. No CI standard, no shared review process, no monitoring beyond what individual projects happened to have.\n\nThe constraints: it had to land on every repo without weeks of per-repo work, it had to hold for repos I rarely open, and it had to police AI-generated code as strictly as mine, because most new code here is written by agents, and an agent will happily add three hundred lines where thirty would do.",
    myRole:
      "I designed the pipeline, wrote the rollout tooling and the fleet scripts, and ran the deployment across the fleet in a day. The templates, the gates, the net-negative rule and the audit cadence are my calls. The heavy lifting inside each run, scanning, reviewing, fixing, is done by agents working inside those rules.",
    decisions: [
      {
        decision:
          "Ship the pipeline as versioned templates plus a one-command rollout script, not as per-repo handiwork.",
        why: "Thirty repos means any manual step happens thirty times, badly. A single command copies the workflow files, review config and docs scaffold into a repo and wires branch protection, so a new repo gets the full floor with one command before its first push. That step is manual, written into my rules as mandatory, not automatic. The standard lives in one place and rolls forward from there.",
        rejected:
          "Configuring each repo by hand as it becomes 'important enough'. That's how the fleet got inconsistent in the first place.",
        tradeoff:
          "Template monoculture: one bad line in the shared template ships to every repo at once. I accepted that risk knowingly, and paid it, which the evals section owns in detail.",
      },
      {
        decision:
        ...
        tradeoff:
          "Some genuinely good additive changes don't fit through the gate and have to come from a human-initiated change instead. That's fine. The loop's job is to shrink entropy, not to build features.",
      },
      {
        decision:
          "Enforce the rules server-side, branch protection and required checks on GitHub, instead of trusting local hooks and good intentions.",
        why: "Local hooks can be skipped with one flag, and an agent or a hurried human will eventually skip them. Server-side protection can't be bypassed from a laptop: the merge simply doesn't happen with a red check. I verified the protection held on every active repo I tested: ten out of ten.",
        rejected:
          "Relying on the pre-commit layer alone. It stays, since it gives fast local feedback, but it's the seatbelt, not the wall.",
        tradeoff:
          "Red CI now genuinely blocks work, including mine, including at the worst possible moment. That's the point, and it's also why the fleet-wide CI failure two weeks later actually hurt.",
      },
      {
        decision:
          "Build uptime monitoring on GitHub Actions instead of buying a monitoring SaaS: a cron pings every client and venture site, opens an issue on downtime, closes it on recovery.",
        why: "The need is real: client sites going down is the one failure a solo operator hears about last, but it's a ten-minute-interval ping, not an observability platform. Actions runs it free on infrastructure I already trust, and downtime lands where I already work: as an issue in a repo.",
        rejected:
          "A paid uptime service. Nothing wrong with them; this just didn't need one more vendor, one more login, one more subscription for a loop I could own end to end.",
        tradeoff:
          "Ten-minute resolution and no fancy dashboards or SMS escalation. For this fleet, an issue notification within minutes is enough; I tested the loop end to end against live sites: four out of four reporting correctly.",
      },
    ],
    build:
      "The rollout day: templates first: a CI quality gate that fails builds on lint, type errors, high-severity CVEs in dependencies and copyleft licenses, plus static analysis; an AI review workflow that reads every pull request; branch protection requiring the checks; and a docs scaffold with a changelog and a per-repo REVIEW-LEARNINGS file, where review findings accumulate so the reviewer gets smarter about each codebase instead of repeating itself.\n\nThen the rollout script walked the fleet and stamped it all in, repo by repo, in one sitting. New repos get the same treatment as a bootstrap step: one command before the first push.\n\nOn top of the per-repo floor sit the fleet jobs: the net-negative improvement loop opening its deletion-biased pull requests, the uptime cron watching the live sites, and a supply-chain scanner that sweeps every skill, plugin and script an agent could execute for indicators of compromise, the kind of thing you want checked precisely because agents install things.\n\nAnd because the pipeline itself is a system that can rot, I scheduled a biweekly self-heal audit over it: verify the hooks parse, the protection is still wired, CI is actually green, the improvement loop is opening sane pull requests, and fix what drifted.",
    evals:
      "The design was checked the day it shipped: server-side protection tested on ten active repos, ten holding; the uptime loop tested end to end, four live sites out of four reporting correctly; the supply-chain scan swept 1,844 files and triaged 51 hits: reviewed, none turned out to be an active compromise.\n\nThen the real eval arrived uninvited. Two weeks after rollout, the scheduled audit ran and reported that CI was red across the fleet: one hundred percent of checked repos failing. The cause was mine: a security hardening in the shared template (pinning a scanner's action to a commit SHA) was wrong, and because every repo shares the template, every repo inherited the breakage at once. The monoculture risk I'd accepted in writing had cashed itself in.\n\nI count that as the system working. A quality gate that can fail loudly, an audit that reports the embarrassing number instead of rounding it away, and a single-file fix that repairs the whole fleet: that's the trade I chose, seen from both sides.",
    limitations:
      "The template monoculture is real and it already bit once. One shared line broke thirty CI pipelines simultaneously. The same property that makes fixes cheap makes mistakes expensive; versioned templates and the audit cadence manage that risk, they don't remove it.\n\nThe pipeline is also young. The net-negative loop ran weekly for only a few weeks. On 9 August 2026 I took it off the schedule, because it was also cleaning old repos nobody had asked it to touch, and I want to police the code being written now. Today I run it by hand on one repo I pick, so I have no long-term evidence of how much entropy it removes. And the whole thing polices code quality, not product quality: a well-linted, CVE-free feature can still be the wrong feature.\n\nFinally, the audit runs on a schedule, so a fleet-wide breakage can live for days before it's caught. Faster detection would mean wiring CI status into the daily audit; it's on the list, honestly, behind things that matter more.",
    results:
      "Every repo in the fleet now has the same floor: gated CI, AI review with per-repo memory, protection that can't be talked past, and an improvement loop whose only allowed direction is smaller and cleaner, now run on demand. New repos start at that floor by default: the standard is a bootstrap command, not a resolution.\n\nOperationally: client and venture sites are watched around the clock by a monitor I own outright, the supply chain of agent-executable code has been swept and is re-swept on schedule, and the one fleet-wide failure so far was caught by the system's own audit and fixed at the template.\n\nWhat I'd show a hiring manager is the shape of it: one person, about thirty repos, and a quality bar that doesn't depend on that person having a good week.",
    principle:
      "At fleet scale, quality is a default, not a virtue. Put the rules server-side where nobody can skip them, ship them as versioned templates so the fix for everything is a fix in one place, and point an audit at your own pipeline, because the system that guards your code will eventually need guarding, and the day it embarrasses you is the day it proves it works.",
    stack: [
      "GitHub Actions (CI quality gate)",
      "AI code review with per-repo learnings",
      "Server-side branch protection",
      "Semgrep + dependency/license scanning",
      "Net-negative auto-improve loop (on demand)",
      "Self-built uptime monitor (Actions cron + Issues)",
