...
export const SECONDARY: SecondaryStudy[] = [
  {
    ...
    title: "One engine ships every Reykjawwwik client site",
    summary:
      "Reykjawwwik ships live client sites across several unrelated verticals off one shared engine. A new brief reaches live fast because the machine does the work, not an all-nighter. I productized the stack into shared starters and a common component system, and gave up some per-site flexibility to get there.",
  ...
  },
  {
    ...
    summary:
      "I built an end-to-end lead engine to fill my own ventures' pipelines. Deliverability was the hard constraint, so every contact gets verified and scored before it enters the queue, and the domain sends on a warmed, throttled cadence instead of one big blast. The payoff isn't a vanity send count; it's a clean, scored queue and a domain that stayed healthy past send one.",
    stack: [
      "Python",
  ...
  },
  {
    ...
    title: "AI ad creatives built for trust, not image quality",
    summary:
      "My first AI ad creatives read as fakes. So I stopped prompting the image model cold and put brand-and-audience research in front of generation, and the drafts came out usable. What fixed it wasn't a better model; it was matching the photography style to what a specific audience reads as credible, with a human still picking the final frame.",
    stack: [
      "Python",
  ...
  },
  {
    ...
    summary:
      "I co-built a glacier-tour operation from nothing into a well-reviewed operator, 1,000+ five-star guests during my time, then handed it off so it kept running after I left. The call that drove it: rent the booking and distribution stack instead of building it, so we could sell before the season's runway ran out.",
    stack: [
  ...
  },
  {
    ...
    summary:
      "Custom-print pricing used to mean a spreadsheet and a promise to email later. Now it's a PWA on the reps' phones: the supplier's price list wired in line-by-line and checked against the source, live currency from the national bank's API, and a margin formula you can defend in front of the client — tax in, margin, tax out, nothing hidden. Behind it sits a 10-step quote-to-delivery pipeline with role-scoped visibility, automatic commissions, and a status-change guard that logs every transition.",
    stack: ["TanStack Start", "React 19", "Supabase", "Postgres RLS", "PWA", "Cloudflare Workers"],
  },
  {
    slug: "fleet-manager-saas-replacement",
    title: "Replacing a rental SaaS with an in-house fleet manager",
    summary:
      "The rental side ran on a paid SaaS we'd outgrown, so I built the replacement. It's a resource-timeline calendar for the whole fleet — reservations, blocks and service on one axis — with insurance and inspection alerts before deadlines bite, client records, and generated rental contracts with handover protocols. The unglamorous part was the point: reconciling the migration and restoring 32 bookings the export had silently dropped, then locking data access server-side with row-level security before anyone trusted it with a season.",
    stack: ["Next.js", "TypeScript", "Supabase", "Postgres RLS", "Vercel", "PWA"],
  },
  {
    slug: "photo-to-3d-site-pipeline",
    title: "A scroll-driven 3D product site, fed by a photo-to-3D pipeline",
    summary:
      "A reusable premium-product site: a glass 3D hero that turns and reveals on scroll, studio lighting and post-processing, running fully offline with no runtime asset fetches. The interesting half is the pipeline behind it. A photograph goes through an image-to-3D model, comes out as a GLB, and gets optimized for the web automatically — plus a written build spec with pinned, verified library versions and a definition of done, so the next build is a repeat, not an adventure.",
    stack: [
      "React Three Fiber",
      "three.js",
      "GSAP",
