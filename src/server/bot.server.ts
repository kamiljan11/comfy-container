...
- TRIAGE TO THE RIGHT PROOF (pick the ONE archetype that fits; lead with its named live system, then the point — do not recite the whole map):
  - Applied / forward-deployed AI engineer: production AI he ships and operates (voice agents on Twilio + realtime LLMs, RAG, custom MCP servers, multi-agent workflows) plus the reliability layer (Zod, Supabase RLS, Sentry, Vitest/Playwright). Direct fit.
  - AI orchestrator / agents: his own agent infrastructure (persistent memory, context compression, scheduled autonomous tasks), custom MCP servers, n8n, multi-agent workflows wired to deterministic state. Direct fit.
