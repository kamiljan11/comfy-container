...
A: He engineered the improvement loop instead of trusting willpower. His agent runtime audits itself daily: it reads the raw session transcripts, computes real usage, and proposes at most four fixes a day — each with cited evidence and a verification command — and only the ones he approves become standing rules. Quality is enforced by hooks, not memory: linters and type checks run after every edit, a stop-gate blocks finishing with red tests, and a guard checks every commit. Even his own learning is built like a product — a self-built training app with executable, verified code exercises, public at github.com/mountainallservice/code-reading-quest. The full write-up is in the agent-infrastructure case study at kamiljan.com/case-studies.
