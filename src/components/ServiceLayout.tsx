...
/**
 * The reading layout behind every sales landing page: /uslugi/$slug today and
 * /obszary/$slug next. It takes a Service and nothing else, so an area page
 * only has to produce the same shape of copy to get the same page.
 *
 ...
 * on top of the site bar. The <main> is kept: grep of src/ shows no other
 * <main> on these routes, so this supplies the one the page is missing.
 *
 ...
 *  ASCII would. ł has no NFD decomposition, hence the explicit replace.
 *  The combining range is written as an escape so the file stays ASCII-safe
 *  for scripts/check-encoding.mjs. */
 ...
 * threshold together, not its static position among its DOM siblings: a card
 * that scrolls into view alone animates immediately instead of sitting still
 * for 280ms because it happens to be the fifth child.
 */
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        // stagger reads as a group entrance rather than as lag
        entries
...
export function ServiceLayout({ s, lang }: { s: Service; lang: Lang }) {
            ...
            <div className="sl-faq">
              {s.faq.map((f, i) => (
                <details
                  ...
                  data-sl-reveal
                >
                  {/* the question is a real h3 so the FAQ shows up in a
                      screen reader's heading list like every other section */}
